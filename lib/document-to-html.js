...
function DocumentConversion(options, comments) {
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        var align = !element.alignment && !!element.styleName && /(HEADING\ [1-2]{1}$)|(.*TITLE)/.test(element.styleName.toLocaleUpperCase()) ? 'center' : element.alignment; // title default align is center
    ...
    function convertRun(run, messages, options) {
        ...
        // ==========================================
        // TAMBAHKAN FONT SIZE DI SINI
        // ==========================================
        if (run.fontSize) {
            paths.push(
                htmlPaths.element(
                    "span",
                    {style: "font-size:" + run.fontSize + "pt"},
                    {fresh: false}
                )
            );
        }
